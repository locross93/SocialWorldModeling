...
""" SGNet adapted from  https://github.com/ChuhuaW/SGNet.pytorch/tree/main """
import copy
...
class BiTraPNP(nn.Module):
    def __init__(self, config):
        ...
        self.config = copy.deepcopy(config)    #copy.deepcopy(args)
        ...
        self.gt_goal_encoder = nn.GRU(input_size=self.pred_dim,
                                        ...
                                        bidirectional=True,
                                        batch_first=True)
        self.p_z_x = nn.Sequential(nn.Linear(self.hidden_size, 128),
                                    nn.ReLU(),
                                    nn.Linear(128, 64),
                                    nn.ReLU(),
                                    nn.Linear(64, self.config["latent_dim"] * 2))
        # posterior
        self.q_z_xy = nn.Sequential(nn.Linear(self.hidden_size + self.hidden_size, 128),
                                    nn.ReLU(),
                                    nn.Linear(128, 64),
                                    nn.ReLU(),
                                    nn.Linear(64, self.config["latent_dim"] * 2))
    ...
    def gaussian_latent_net(self, enc_h, cur_state, K,  target=None, z_mode=None):
        ...
        z_mu_p = z_mu_logvar_p[:, : self.config["latent_dim"]]
        z_logvar_p = z_mu_logvar_p[:, self.config["latent_dim"] :]
        if target is not None:
            ...
            z_mu_q = z_mu_logvar_q[:, : self.config["latent_dim"]]
            z_logvar_q = z_mu_logvar_q[:, self.config["latent_dim"] :]
            Z_mu = z_mu_q
            Z_logvar = z_logvar_q
...
class FeatureExtractor(nn.Module):
    def __init__(self, config: Dict):
        super().__init__()
        self.config = config
        self.feature_extractor = nn.Sequential(nn.Linear(
            config['obs_size'], config['hidden_size']),
            nn.ReLU())
...
class SGNet_CVAE(nn.Module):
    def __init__(self, config: Dict):
        ...
        self.pred_dim = config['pred_dim']    # predicted states dimension
        self.K = config['K']
        self.map = config['map']
        ...
        self.goal_hidden_to_traj = nn.Sequential(nn.Linear(self.hidden_size//4, self.hidden_size),
                                                 nn.ReLU(inplace=True))
        self.cvae_to_dec_hidden = nn.Sequential(nn.Linear(self.hidden_size + self.config['latent_dim'], self.hidden_size),
                                                nn.ReLU(inplace=True))
