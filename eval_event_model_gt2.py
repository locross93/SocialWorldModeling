...
class Analysis(object):
    ...
    def eval_goal_events_in_rollouts(self, model, input_data, level=1, partial=1.0) -> Dict[str, typing.Any]:
        ...
        for split in ['train', 'val']:
            ...
            for i in range(self.exp_info_dict[split]['pickup_timepoints'].shape[0]):
                all_event_inds = np.concatenate([self.exp_info_dict[split]['pickup_timepoints'][i,:],self.exp_info_dict[split]['goal_timepoints'][i,:]])
        ...
        for i,row in enumerate(single_goal_trajs):
            ...
            assert len(next_events_inds) > 0
            assert np.min(next_events_inds) <= (self.num_timepoints - 1)
            ...
            if model.mp_model.input_pred_horizon:
